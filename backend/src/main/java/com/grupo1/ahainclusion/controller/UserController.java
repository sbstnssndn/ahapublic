...
public class UserController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignUpRequest signUpRequest) {
        ...
        if(userRepository.existsByEmail(signUpRequest.getEmail())) {
            return new ResponseEntity(new ApiResponse(false, "Email ya en uso!"),
        ...
        if((isCandidato || isEmpresa || isAHA)==false) {
            return new ResponseEntity(new ApiResponse(false, "Rol invalido"),
                    HttpStatus.BAD_REQUEST);
        }

        if(isCandidato) userRole = roleRepository.findByName("ROLE_CANDIDATO");
        if(isEmpresa) userRole = roleRepository.findByName("ROLE_EMPRESA");
        if(isAHA) userRole = roleRepository.findByName("ROLE_AHA");
